...
export const radiologyStudiesTable = pgTable(
  "radiology_studies",
  {
    ...
    pacsArchiveStatus: text("pacs_archive_status").notNull().default("none"), // none | pending | success | failed
    pacsArchiveResponse: text("pacs_archive_response"),
...
export const radiologyStudyLocksTable = pgTable(
  "radiology_study_locks",
  {
    ...
    lockTime: timestamp("lock_time", { withTimezone: true }).notNull().defaultNow(),
    lastActivityAt: timestamp("last_activity_at", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()),
    workstation: text("workstation"),
  },
  (t) => ({
    studyIdUq: uniqueIndex("radiology_study_locks_study_id_uq").on(t.studyId),
    byStudyId: index("radiology_study_locks_study_id_idx").on(t.studyId),
    byUserId: index("radiology_study_locks_user_id_idx").on(t.userId),
  })
);

export type RadiologyStudyLock = typeof radiologyStudyLocksTable.$inferSelect;

export const radiologyChocolateFindingsTable = pgTable(
...
export type RadiologyChocolateFinding = typeof radiologyChocolateFindingsTable.$inferSelect;

export const radiologyUserFindingsPreferencesTable = pgTable(
  "radiology_user_findings_preferences",
  {
    id: serial("id").primaryKey(),
    userId: integer("user_id").notNull(),
    favoriteFindingIds: text("favorite_finding_ids").notNull().default("[]"),
    customFindingsJson: text("custom_findings_json").notNull().default("[]"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()),
  }
);

export type RadiologyUserFindingsPreference = typeof radiologyUserFindingsPreferencesTable.$inferSelect;

export const radiologyUserReportPreferencesTable = pgTable(
  "radiology_user_report_preferences",
  {
    id: serial("id").primaryKey(),
    userId: integer("user_id").notNull(),
    favoriteFindings: text("favorite_findings").notNull().default("[]"),
    favoriteImpressions: text("favorite_impressions").notNull().default("[]"),
    favoriteTemplates: text("favorite_templates").notNull().default("[]"),
    personalMacros: text("personal_macros").notNull().default("[]"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()),
  }
);

export type RadiologyUserReportPreference = typeof radiologyUserReportPreferencesTable.$inferSelect;

export const radiologyUserItemUsageLogsTable = pgTable(
...
export type RadiologyUserItemUsageLog = typeof radiologyUserItemUsageLogsTable.$inferSelect;
