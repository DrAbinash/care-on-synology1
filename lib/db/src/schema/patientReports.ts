...
export const patientReportsTable = pgTable(
  "patient_reports",
  {
    ...
    // Denormalized cache of this report's report_finding_instances rows
    // (Radiology Roadmap Ticket A2). Nullable — regenerated by a later
    // ticket, never a source of truth. `body` remains the authoritative,
    // append-only signed artifact; these three version columns exist so a
    // future ticket can freeze exactly which renderEngine/template/catalog
    // version produced `body` at signature, without touching `body` itself.
    // Not read or written by any route yet.
