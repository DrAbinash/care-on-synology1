...
-- =============================================================================
--
-- This file consolidates ALL 179 ADD COLUMN statements that were previously
-- scattered inside runStartupMigrations() in artifacts/api-server/src/index.ts.
--
-- WHY THIS EXISTS:
--   runStartupMigrations() runs WHEN the API starts. But the API depends on
--   these columns existing BEFORE it can start (for Drizzle ORM inserts,
--   for the schema verifier, for seedBootstrapAdmin). This circular dependency
--   caused repeated deployment failures.
--
-- This migration runs in db-patch-v2, BEFORE the API starts, permanently
-- breaking the circular dependency.
--
-- ALL statements use ADD COLUMN IF NOT EXISTS — 100% idempotent.
-- Safe on fresh databases and on already-running production databases.
-- runStartupMigrations() in index.ts becomes a no-op for schema changes
-- (the IF NOT EXISTS ensures no conflict).
-- =============================================================================

-- ── ai_normal_report_templates ────────────────────────────────────────────────

ALTER TABLE ai_normal_report_templates
...
ALTER TABLE kiosk_payment_sessions
  ADD COLUMN IF NOT EXISTS patient_details TEXT NOT NULL DEFAULT '{}';
