...
--     almost no typing.
--
-- GUARD NOTE: This migration ONLY runs if radiology_quick_findings and
-- radiology_study_tabs tables exist. This guards against fresh database
-- scenarios where these tables may not yet be created by earlier migrations.
--
-- Fully idempotent:
--   - Column alterations use "ADD COLUMN IF NOT EXISTS"
--   - Updates are guarded with "(properties IS NULL OR properties = '')"
--   - Admin edits are never overwritten
-- =============================================================================

-- Guard: Only proceed if both required tables exist
DO $$
BEGIN
  ...
  IF EXISTS (
    ...
    -- Add properties column to quick findings
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'radiology_study_tabs'
  ) THEN
    -- Add technique_text and normal_text columns to study tabs
    ALTER TABLE radiology_study_tabs 
      ADD COLUMN IF NOT EXISTS technique_text TEXT NOT NULL DEFAULT '';
    ALTER TABLE radiology_study_tabs 
      ADD COLUMN IF NOT EXISTS normal_text TEXT NOT NULL DEFAULT '';
  ELSE
    RAISE WARNING 'Table radiology_study_tabs does not exist yet; skipping text column additions';
  END IF;
END $$;

-- ── Property chip defaults (guarded — never overwrite admin edits) ───────────
-- Only update if the table exists and rows exist
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
