  ...
  try {
    ...
    if (verification.success && verification.status === "paid") {
      ...
      try {
        ...
        logger.error({ err, bookingId: booking.id }, "Auto-confirmation failed for paid ICICI booking");
      }
      ...
      res.redirect(getBookingConfirmationUrl(base, booking.bookingRef, "icici", true, verification.errorMessage || "Payment not completed"));
      return;
    }
  } catch (err: any) {
    logger.error({ err, merchantTxnNo }, "ICICI callback verification exception");
    res.redirect(getBookingConfirmationUrl(base, booking.bookingRef, "icici", true, err.message || "Payment verification error"));
    return;
  }
}
