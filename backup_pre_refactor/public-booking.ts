...
publicBookingRouter.post("/phonepe-initiate", createOrderLimiter, async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!rpRes.ok) {
      ...
      res.status(400).json({ error: "PhonePe gateway error. Please try again.", details: errText });
      return;
    }
    ...
    if (!rpData.success || !rpData.data?.instrumentResponse?.redirectInfo?.url) {
      res.status(400).json({ error: "Could not initiate PhonePe payment. Please try again.", code: rpData.code });
      return;
    }
...
publicBookingRouter.post("/bharatpe-initiate", createOrderLimiter, async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!bpRes.ok) {
      ...
      res.status(400).json({ error: "BharatPe gateway error. Please try again.", details: errText });
      return;
    }
    ...
    if (!bpData.success || !bpData.data?.redirectUrl) {
      res.status(400).json({ error: "Could not initiate BharatPe payment. Please try again.", code: bpData.code });
      return;
    }
...
function getIciciPrefix() {
  const prefix = process.env.ICICI_URL_PREFIX !== undefined ? process.env.ICICI_URL_PREFIX : "/tsp";
  if (!prefix) return "";
...
publicBookingRouter.post("/icici-initiate", createOrderLimiter, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const prefix = getIciciPrefix();
    const iciciUrl = `${iciciBase}${prefix}/pg/api/v2/initiateSale`;
    
    // safe diagnostic logs
    logger.info({
      iciciBaseUrl: iciciBase,
      initiateSaleEndpoint: `${prefix}/pg/api/v2/initiateSale`,
      merchantId,
      aggregatorId,
      returnURL: returnUrl,
      callbackURL: returnUrl,
      currentRequestHost: req.headers["host"] || "unknown",
      currentPublicBaseUrl: base,
      nodeEnv: process.env.NODE_ENV,
      iciciMode: process.env.NODE_ENV === "production" ? "production" : "uat/test",
      bookingRef
    }, "ICICI initiateSale request details");
    ...
    const resText = await iciciRes.text();
    logger.info({ status: iciciRes.status, responseText: resText }, "ICICI raw response text");
    ...
    try {
      if (resText.trim().startsWith("{")) {
        ...
        logger.error({ status: iciciRes.status, text: resText }, "ICICI returned non-JSON response");
      }
    } catch (parseErr) {
      logger.error({ parseErr, text: resText }, "Failed to parse ICICI response as JSON");
    }
    ...
    // Save to last transaction cache
    lastIciciTransaction = {
      txnRef: bookingRef,
      responseCode: iciciData.responseCode,
      respDescription: iciciData.respDescription,
      timestamp: new Date().toISOString(),
    };

    if (!iciciRes.ok || !iciciData.tranCtx || iciciData.responseCode !== "R1000") {
      res.status(400).json({ error: "Could not initiate ICICI payment. Please try again.", details: iciciData.respDescription || iciciData.responseCode });
      return;
    }
...
async function handleIciciCallback(req: any, res: any, queryOrBody: Record<string, string>): Promise<void> {
  ...
  if (secretKey && merchantId) {
    try {
      ...
      const statusRes = await fetch(`${getIciciBase()}${getIciciPrefix()}/pg/api/command`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({
          merchantId,
  ...
  if (booking.status === "pending_payment") {
    ...
    await db.update(onlineBookingsTable)
      .set({ status: "payment_failed", failureReason: respDescription || "Payment not completed" })
      .where(eq(onlineBookingsTable.id, booking.id));
  }
