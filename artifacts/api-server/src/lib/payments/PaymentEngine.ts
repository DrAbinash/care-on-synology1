...
import { HdfcPaymentProvider } from "./HdfcPaymentProvider";
...
export class PaymentEngine {
  static async getProvider(gatewayId?: string): Promise<PaymentProvider> {
    ...
    const cleanVal = (dbVal?: string | null, envVal?: string) => 
      (dbVal && dbVal !== "NA" && dbVal !== "null" && dbVal.trim() !== "") ? dbVal : (envVal || "");

    switch (activeGateway) {
      case "icici":
        return new IciciPaymentProvider({
          merchantId: cleanVal(settings?.iciciMerchantId, process.env.ICICI_MERCHANT_ID),
          aggregatorId: cleanVal(settings?.iciciAggregatorId, process.env.ICICI_AGGREGATOR_ID),
          secretKey: cleanVal(settings?.iciciSecretKey, process.env.ICICI_SECRET_KEY),
          baseUrl: process.env.ICICI_BASE_URL || undefined,
          urlPrefix: process.env.ICICI_URL_PREFIX,
        });
      ...
      case "phonepe":
        return new PhonePePaymentProvider({
          merchantId: cleanVal(settings?.phonepeMerchantId, process.env.PHONEPE_MERCHANT_ID),
      ...
      case "bharatpe":
        return new BharatPePaymentProvider({
          merchantId: cleanVal(settings?.bharatpeMerchantId, process.env.BHARATPE_MERCHANT_ID),
      ...
      case "payu":
        return new PayUPaymentProvider({
          merchantKey: cleanVal(settings?.payuMerchantKey, process.env.PAYU_MERCHANT_KEY),
          merchantSalt: process.env.PAYU_MERCHANT_SALT || "",
        });

      case "razorpay":
        return new RazorpayPaymentProvider({
          keyId: cleanVal(settings?.razorpayKeyId, process.env.RAZORPAY_KEY_ID),
          keySecret: process.env.RAZORPAY_KEY_SECRET || "",
        });

      case "cashfree":
        return new CashfreePaymentProvider({
          appId: cleanVal(settings?.cashfreeAppId, process.env.CASHFREE_APP_ID),
  ...
  static async initiatePayment(gatewayId: string, params: InitiatePaymentParams): Promise<InitiatePaymentResult> {
    console.log("PAYMENT ENGINE START: " + params.bookingRef);
