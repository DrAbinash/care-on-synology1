 ...
 * Degree comes from Settings → Doctors (doctors master), never from a local add form.
 */
...
export default function ReferringDoctorQuickSelect({
  ...
  const chips = useMemo(() => {
    ...
    for (const id of slotIds) {
      ...
      seen.add(key);
      out.push({ name: label, source: "quick" });
    }
    ...
    for (const f of frequent?.doctors ?? []) {
      ...
      seen.add(key);
      const match = doctors.find((d) => matchKey(d.name) === key);
      out.push({ name: match ? catalogLabel(match) : formatDoctorWithDegree(f.name), source: "frequent" });
    ...
    <div className="col-span-2 mt-0.5 space-y-1.5" data-testid="ref-doctor-quick-select">
        ...
        {chips.map((c) => {
          ...
          return (
            <button
              ...
              className={[
                "max-w-[11rem] truncate rounded border px-1.5 py-0.5 text-[10px] font-medium transition-colors",
                selected
      ...
      {chips.length === 0 && !searchOpen && (
        <p className="text-[10px] text-muted-foreground">
          Pin doctors on Billing Desk quick slots, or use More to search. Add degrees in Settings → Doctors.
        </p>
      )}
    </div>
  );
