      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <AlertTriangle className="h-4 w-4 text-amber-600" /> Overwrite existing content?
          </DialogTitle>
          <DialogDescription>The editor has content. Applying will replace it.</DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          <div className="rounded-md border border-amber-300 bg-amber-50 p-2.5 text-xs text-amber-800">
            ...
            {!pendingPatch && <div className="mt-1 text-[10px] text-amber-700/80 italic">Tip: Cancel and "Save as Format" first to preserve current work.</div>}
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={confirm} className="bg-amber-600 hover:bg-amber-700">Overwrite & apply</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
