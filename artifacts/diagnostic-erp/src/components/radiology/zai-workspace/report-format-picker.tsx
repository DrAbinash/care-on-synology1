...
export function ReportFormatPicker() {
  ...
  const lookedUp = useMemo(
    () => lookupFormatsForPicker(formats, study?.modality, reportingContext, {
      protocolName: reportingContext.protocolName,
      studyDescription: reportingContext.studyDescription ?? study?.studyDescription,
      bodyPartFallback: study?.bodyPart ?? null,
    }),
    [formats, study?.modality, study?.studyDescription, study?.bodyPart, reportingContext],
  );
  const scoped = lookedUp.formats;
  ...
  if (!study) return <div className="rounded border border-dashed border-emerald-300/60 bg-emerald-50/30 p-2 text-[11px] text-emerald-700/80 text-center" data-testid="report-format-needs-study">Select a study</div>;
  const regionLabel = reportingContext.region || study.bodyPart || "unresolved";
  return (
    <div className="space-y-2" data-testid="full-report-format-picker" data-format-scope={lookedUp.scope}>
      <div className="flex items-center justify-between"><div className="text-[10px] font-semibold uppercase tracking-wider text-emerald-600/80">Full Report Formats</div><span className="rounded bg-emerald-50 border border-emerald-200/60 px-1 py-0.5 font-mono text-[9px] text-emerald-700">{study.modality} · {regionLabel}</span></div>
      ...
      {lookedUp.scope === "modality" && (
        <div className="rounded border border-amber-300/70 bg-amber-50/80 px-2 py-1 text-[10px] text-amber-900" data-testid="report-format-modality-fallback">
          Region not matched yet — showing all {study.modality} formats. Set Region/Protocol above to narrow.
        </div>
      )}
      {lookedUp.scope === "bodyPart" && !reportingContext.region && (
        <div className="rounded border border-sky-300/70 bg-sky-50/80 px-2 py-1 text-[10px] text-sky-900" data-testid="report-format-bodypart-fallback">
          Using study body part ({study.bodyPart}) until DICOM region resolves.
        </div>
      )}
