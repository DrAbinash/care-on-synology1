...
export function QuickSelectStrip({ field }: { field: QuickSelectField }) {
  ...
  if (!study) return <div className="mb-2 text-[10px] text-muted-foreground"><span className="font-semibold uppercase tracking-wider">{LABELS[field]} Quick Select · select a study</span></div>;
  return (
    <div className="mb-2">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-1.5 text-[10px]"><span className="font-semibold uppercase tracking-wider text-muted-foreground/80">{LABELS[field]} Quick Select</span><span className="rounded bg-muted/60 px-1 py-0.5 font-mono text-[9px] text-muted-foreground">{study.modality} · {study.bodyPart}</span><span className="text-muted-foreground/60">{scoped.length} tiles</span></div>
      ...
      <div className="flex flex-wrap gap-1 max-h-[88px] overflow-y-auto pr-1">
        {filtered.map(tile => <div key={tile.id} className="group relative inline-flex items-center gap-1 rounded-md border bg-card px-2 py-1 text-[10.5px] cursor-pointer transition hover:border-emerald-300 hover:shadow-sm" onClick={() => { const c = useWorkspace.getState()[`${field}Text` as "findingsText"] as string; const s = c.length > 0 && !c.endsWith(" ") && !c.endsWith("\n") ? " " : ""; useWorkspace.getState().setField(field, c + s + tile.sentence + " "); incUsage(tile.id); }} title={tile.sentence}>
          ...
          {tile.usageCount && tile.usageCount > 3 && <span className="rounded bg-emerald-50 text-emerald-600 px-0.5 text-[8px] font-mono">{tile.usageCount}</span>}
        ...
        <button onClick={() => openEditor(null, field)} className="inline-flex items-center gap-1 rounded-md border-2 border-dashed border-muted-foreground/30 bg-muted/20 px-2 py-1 text-[10.5px] text-muted-foreground hover:border-emerald-400 hover:text-emerald-600 hover:bg-emerald-50/30 transition"><Plus className="h-3 w-3" /><span>Add tile</span><ChevronRight className="h-2.5 w-2.5 opacity-50" /></button>
      </div>
    </div>
  );
}
