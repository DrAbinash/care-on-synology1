...
export default function QueueDisplay() {
  ...
  const rawRoomKey = (params.roomKey || search.get("roomKey") || "usg").toLowerCase();
  const roomKey = rawRoomKey.replace(/-?room$/, "") || rawRoomKey;
  ...
  const buildQueueUrl = useCallback(
    (path: string) => {
      ...
      u.set("ledgerId", String(settings?.ledgerId ?? 1));
      if (settings?.departments) u.set("departments", settings.departments);
      if (displayToken) u.set("displayToken", displayToken);
      return `/api/display/${path}?${u.toString()}`;
    },
    [settings?.ledgerId, settings?.departments, displayToken],
  );

  const { data: queueData } = useQuery<DisplayPayload>({
    queryKey: ["queue-display-feed", roomKey, settings?.ledgerId, settings?.departments, displayToken],
    enabled: !!settings,
  ...
  useEffect(() => {
    ...
    const es = new EventSource(buildQueueUrl("queue-stream"));
    es.onmessage = (evt) => {
      if (!evt.data || evt.data.startsWith(":")) return;
      try {
        const payload = JSON.parse(evt.data) as DisplayPayload;
        qc.setQueryData(
          ["queue-display-feed", roomKey, settings.ledgerId, settings.departments, displayToken],
          payload,
        );
    ...
    es.onerror = () => es.close();
    return () => es.close();
  }, [settings, roomKey, displayToken, buildQueueUrl, qc]);
