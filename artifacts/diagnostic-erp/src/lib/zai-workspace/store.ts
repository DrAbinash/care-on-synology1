...
export const useWorkspace = create<S & {
  ...
  setEditorContent: (c) => set({ findingsText: c.findings, impressionText: c.impression, recommendationText: c.recommendation, techniqueText: c.technique, clinicalHistoryText: c.clinicalHistory, isDirty: true }),
  ...
  advanceToNextStudy: () => { const { studies, completedStudyIds, activeStudyId, nextStudyId, nextStudyPreloaded } = get(); if (nextStudyId && nextStudyPreloaded) { get().selectStudy(nextStudyId); set({ nextStudyId: null, nextStudyPreloaded: false }); return; } const r = studies.filter(s => !completedStudyIds.has(s.id) && s.id !== activeStudyId); const pr: Record<string, number> = { stat: 0, urgent: 1, routine: 2, vip: 1 }; r.sort((a, b) => (pr[a.priority] - pr[b.priority]) || (a.tatMinutes - b.tatMinutes)); if (r[0]) get().selectStudy(r[0].id); else set({ activeStudyId: null, findingsText: "", impressionText: "", recommendationText: "" }); },
