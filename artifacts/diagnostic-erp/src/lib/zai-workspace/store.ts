...
import { coverageMarksEqual, defaultCoverageMarks, setCoverageStatus, parseCoverageMarks, COVERAGE_ENVELOPE_KEY } from "@/lib/coverageMarks";
...
interface S {
  ...
  activeAnchor: ObservationAnchor | null;
  /** Advisory coverage marks (MRI lumbar). Never a finalize hard gate. */
...
const createWorkspaceStore: StateCreator<WorkspaceStore> = (set, get) => ({
  ...
  activeAnchor: null, coverageMarks: [],
  ...
  serializeObservationLedger: () => serializeObservationLedger(
    ...
    get().fieldProvenance,
    get().coverageMarks.length > 0 ? { [COVERAGE_ENVELOPE_KEY]: get().coverageMarks } : undefined,
  ),
  ...
  setCoverageMark: (regionKey, status, reason) => {
    const base = get().coverageMarks.length ? get().coverageMarks : defaultCoverageMarks();
    const next = setCoverageStatus(base, regionKey, status, reason);
    if (coverageMarksEqual(get().coverageMarks, next)) return;
    set({ coverageMarks: next, isDirty: true });
  },
  hydrateCoverageMarks: (raw) => {
    const parsed = parseCoverageFromRaw(raw);
    if (parsed) set({ coverageMarks: parsed });
  },
