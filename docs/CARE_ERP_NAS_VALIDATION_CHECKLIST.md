...
| 10b | **Evolution API (deprecated, optional)** | `curl -I $EVOLUTION_API_URL` (only if this legacy probe is still configured) | 2xx/3xx | unreachable — Evolution is not CARE's WhatsApp provider, safe to ignore/unset `EVOLUTION_API_URL` | n/a — never blocks a deploy |
