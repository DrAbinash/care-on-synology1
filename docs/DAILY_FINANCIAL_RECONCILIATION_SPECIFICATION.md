...
**Follow-up (same day): `auto-voucher.ts`'s Tally ledger-account posting had the identical "unknown → cash" defect, separately.** This module is not part of the reconciliation math above — it decides which Tally/accounting ledger account a payment or expense posts to (`Cash in Hand`, `UPI Collections`, etc.) — but it had its own independent copy of the same exact-match bug: `METHOD_ACCOUNTS[methodKey] ?? METHOD_ACCOUNTS["cash"]`. A gateway-qualified string like `"Online (ICICI Orange Pay)"` failed the exact match and posted to `Cash in Hand` in Tally, and `"insurance"` had no entry at all and did the same. Fixed via `resolveMethodAccount()`: exact match first (preserving every existing method's historical account name, including keeping `bank`/`neft`/`rtgs` as their own separate accounts rather than merging them into `Online Collections`), then falls back to the shared classifier only for methods that don't exactly match. Two new accounts were added: `Insurance Collections` (previously missing) and `Unclassified Collections (Needs Review)` (`bank`-typed, so it never inflates the `Cash in Hand` balance) for anything the classifier still can't recognize.
