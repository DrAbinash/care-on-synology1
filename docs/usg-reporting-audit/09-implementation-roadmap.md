...
1. **Fix the obstetric GA/EFW unit-mismatch bugs** (doc 06, §3) in `fetalUsgLevel4.ts` — this is a live patient-safety-adjacent bug independent of everything else in this audit and should not wait for any architecture decision. Touches only USG-specific files (🟢 safe tier).
